...
public:
	bool queryImage(int uid, int act_id);
	bool queryTxt(int act_id);

	bool queryBeforeGet(int uid, int act_id);
	cv::Mat getImageFromDatabase(int uid, int act_id);
	std::string getTxtFromDatabase(int act_id);

	bool writeScoreToDatabase(const double score);

	bool writeImageUrlToDatabase(const std::string& imgurl);
	bool writeTxtUrlToDatabase(const std::string& txtUrl);
	bool writePoseDataToDatabase(const std::string& data);
